...
def pingpong(n):
    ...
    True
    """
    def inner(way, current_n, target_n, result):
        if current_n == target_n:
            return result
        if current_n % 8 == 0 or num_eights(current_n) != 0:
            return inner(way * (-1), current_n + 1, target_n, result + way * (-1))
        return inner(way, current_n + 1, target_n, result + way)
    return inner(1, 1, n, 1)
...
def count_coins(change):
    ...
    True
    """
    def inner(change, current_coin):
        if current_coin == 1:
            return 1
        elif current_coin == None:
            return 0
        elif change < current_coin:
            return count_coins(change)
        else:
            return inner(change - current_coin, current_coin) + inner(change, next_smaller_coin(current_coin))
    if change < 5:
        return 1
    elif change >= 5 and change < 10:
        return count_coins(change - 5) + inner(change, 1)
    elif change >= 10 and change < 25:
        return count_coins(change - 10) + inner(change, 5)
    else:
        return count_coins(change - 25) + inner(change, 10)
