...
def accuracy(typed, source):
    ...
    # BEGIN PROBLEM 3
    length = min(len(source_words), len(typed_words))
    i, n, r = 0, 0, 0.0
    for i in range(length):
        if source_words[i] == typed_words[i]:
          n += 1
    if len(typed_words) != 0:
        r = 100 * (n / len(typed_words))
...
def wpm(typed, elapsed):
    ...
    # BEGIN PROBLEM 4
    wpm = len(typed) * (60 / elapsed) / 5
    if len(typed) * (60 / elapsed) % 5 == 0:
        return round(wpm, 1)
    return wpm
    # END PROBLEM 4
...
def feline_fixes(typed, source, limit):
    ...
    """
    # BEGIN PROBLEM 6
    typed_length, source_length = len(typed), len(source)
    length = min(typed_length, source_length)
    def inner(current_position, result):
        if result > limit:
            return result
        elif current_position >= length:
            return result
        else:
            if typed[current_position] != source[current_position]:
              return inner(current_position + 1, result + 1)
            return inner(current_position + 1, result)
    return inner(0, abs(typed_length - source_length))
    # END PROBLEM 6
